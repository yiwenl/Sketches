...
// --- NEW SEDIMENT UNIFORMS ---
uniform float uSedimentCapacity; // k_c
uniform float uErosionRate;      // k_e
uniform float uDepositionRate;   // k_d
uniform float uMinVolume;        // Check for dryness
...
void main(void) {
    ...
    // Store original water for evaporation scaling later
    ...
    vec2 newPos = pos + newDir * uTimeStep * 0.01;
    
    
    // --- 1. Sediment Calculation (Erosion/Deposition) ---

    // The carrying capacity (C) is proportional to speed/slope and water volume
    float capacity = max(0.0, speed * water * uSedimentCapacity);  
    
    // Calculate the difference between current load and max capacity
    float sedimentDiff = capacity - sediment;
    float deltaSediment = 0.0;

    // Check if we should erode (sedimentDiff > 0) or deposit (sedimentDiff < 0)
    if (sedimentDiff > 0.0) {
        // EROSION: Pick up dirt
        // Erosion rate is proportional to the difference and the available water.
        deltaSediment = uErosionRate * sedimentDiff * water * uTimeStep;
        
        // Ensure we don't erode more than the current height (optional, but safer)
        // Note: Actual heightmap update is done in a separate pass, 
        // this value is for bookkeeping:
        deltaSediment = min(deltaSediment, h_c); 
        
    } else {
        // DEPOSITION: Drop off dirt
        deltaSediment = uDepositionRate * sedimentDiff * uTimeStep; // sedimentDiff is negative here
    }
    
    // Update the droplet's carried sediment
    ...
    if (isOutOfBounds || isDry) {
        newPos = randomPosition(vTextureCoord + pos * 0.001) * 0.5;
        water = uInitialWater; 
        sediment = 0.0;        
        newDir = vec2(0.0, 0.0);
    }
    ...
    oColor2 = vec4(gradient, sedimentDiff, 1.0);
}