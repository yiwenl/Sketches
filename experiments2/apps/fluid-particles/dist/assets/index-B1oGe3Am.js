...
`,Nt=`${Re}\n${At}`,Pt=`${Re}\n${Ot}`,Q=3e5,Ft=1e5,It=256,$=9,Lt=new Float32Array([0,1,0,-.8660254,-.5,0,.8660254,-.5,0]),Rt=$*2,zt=1024,Bt=1.35,Vt=.75,Ht=.65,Ut=.002,Wt=[1,18,8],Gt=[0,0,-1],Kt=$*.5,qt=6,Jt=Kt*.8,Yt=Kt*2.8,Xt=.34,Zt=.6,Qt=.42,$t=Math.PI/180,en={maxRadius:$*1.25,fluidForceScale:2.8,densityForceScale:.02,damping:.984,centerForce:8.4},tn={time:`f32`,dt:`f32`,maxRadius:`f32`,count:`u32`,fluidForceScale:`f32`,densityForceScale:`f32`,damping:`f32`,centerForce:`f32`},nn=K.uniformSize(J.create(tn).byteSize),rn=e=>J.create(tn).set(`time`,0).set(`dt`,1/60).set(`maxRadius`,en.maxRadius).set(`count`,e).set(`fluidForceScale`,en.fluidForceScale).set(`densityForceScale`,en.densityForceScale).set(`damping`,en.damping).set(`centerForce`,en.centerForce);function an(e){let t=window.devicePixelRatio||1;return[Math.max(1,Math.floor(e.clientWidth*t)),Math.max(1,Math.floor(e.clientHeight*t))]}function on(e){let[t,n]=an(e);e.width=t,e.height=n}function sn(e,t){let n=Math.cos(t),r=Math.sin(t);return[e[0],e[1]*n-e[2]*r,e[1]*r+e[2]*n]}function cn(e){let t=Math.hypot(e[0],e[1],e[2])||1;return[e[0]/t,e[1]/t,e[2]/t]}async function ln(){await Ke(),Dt.init();let e=document.createElement(`canvas`);e.className=`app-canvas`,document.body.appendChild(e),on(e);let t=document.createElement(`div`);t.textContent=`${Q.toLocaleString()} fluid particles`,t.className=`particle-count-label`,document.body.appendChild(t);let{fluidTextureSize:n,advectionScale:r,curl:i,densityDissipation:a,velocityDissipation:o,pressureIterations:s}=Z,c=await Se.create(e),l=new ut(c,{TEXTURE_SIZE:n,DENSITY_DISSIPATION:a,VELOCITY_DISSIPATION:o,PRESSURE_DISSIPATION:.95,PRESSURE_ITERATIONS:s,CURL:i,ADVECTION_SCALE:r},$),u=new pt(c,{texSize:n,volumeExtent:Rt}),d=new pt(c,{texSize:n,volumeExtent:Rt}),f=St({count:Q,radius:$}),p=St({count:Ft,radius:$,baseScale:1}),m=[K.fromData(c,f,G.storage,`particles-a`),K.fromData(c,f,G.storage,`particles-b`)],h=[K.fromData(c,p,G.storage,`cubes-a`),K.fromData(c,p,G.storage,`cubes-b`)],g=K.fromData(c,Lt,G.vertex,`particle-triangle-positions`),_=new ze(Lt.length/3).addVertexBuffer({buffer:g,arrayStride:12,attributes:[{shaderLocation:0,format:`float32x3`,offset:0}],slot:0,stepMode:`vertex`}),v=We.cube({size:2}),y=K.fromData(c,v.positions,G.vertex,`cube-positions`),b=K.fromData(c,v.normals,G.vertex,`cube-normals`),x=K.fromData(c,v.indices,G.index,`cube-indices`),S=new ze(v.positions.length/3).addVertexBuffer({buffer:y,arrayStride:12,attributes:[{shaderLocation:0,format:`float32x3`,offset:0}],slot:0,stepMode:`vertex`}).addVertexBuffer({buffer:b,arrayStride:12,attributes:[{shaderLocation:1,format:`float32x3`,offset:0}],slot:1,stepMode:`vertex`}).setIndexBuffer(x,v.indices.length,v.indices instanceof Uint32Array?`uint32`:`uint16`),C=K.create(c,K.uniformSize(ye.uniformByteSize()),G.uniform,`camera-uniforms`),w=new Float32Array(ye.uniformFloatCount),T=new ye(45*$t,1,.1,300),E=new le(T,{listenerTarget:e,center:[0,0,0],radius:$*4,sensitivity:1,zoomSpeed:.8,panSpeed:.02});E.rx.setTo(-.22),E.ry.setTo(.72);let D=new he(We.sphere({radius:Kt,segments:24}),T,[e.width,e.height],{listenerTarget:e}),O=Z,k=!0,A=ht();D.addEventListener(`onHit`,(e=>{let t=e.detail.hit;if(k){vt(A,t),k=!1;return}let n=ht();xt(n,t,A);let r=_t(n);if(r>.001&&r<$){let e=gt(n);bt(e,e);let i=O.strength*(r*500);l.addForce([t[0],t[1],t[2]],[e[0],e[1],e[2]],O.radius,i,1,O.noiseStrength)}vt(A,t)}));let ee=e=>{let t=e*Qt,n=O.strength*Xt;for(let e=0;e<qt;e++){let r=e/qt*Math.PI*2,i=Math.cos(r),a=Math.sin(r),o=Math.random()*.5+.5,s=[i*Jt*o,0,a*Jt*o],c=[-a,0,i],u=[-i,0,-a],d=cn([c[0]+u[0],c[1]+u[1],c[2]+u[2]]);l.addForce(sn(s,t),sn(d,t),Yt,n,Zt,O.noiseStrength)}},j=new be(-1,1,-1,1,.1,100);xe({camera:j,center:[0,0,0],radius:Tt({maxRadius:$,overshootMultiplier:Bt,billboardPadding:Vt})+1,eye:Wt,up:Gt,padding:1});let M=K.create(c,K.uniformSize(be.uniformByteSize()),G.uniform,`light-camera-uniforms`),N=new Float32Array(be.uniformFloatCount);j.writeUniformData(N),M.write(c,N);let P=new be(-1,1,-1,1,.1,100);xe({camera:P,center:[0,0,0],radius:$,eye:[0,0,$*3],up:[0,1,0]});let F=K.create(c,nn,G.uniform,`particle-sim-params`),te=K.create(c,nn,G.uniform,`cube-sim-params`),ne=rn(Q),re=rn(Ft),ie=new Y(c,Mt,{label:`ParticlesUpdate`,entryPoint:`cs_main`}),I=(e,t,n,r,i)=>q.create(c,ie.getBindGroupLayout(0),[{binding:0,resource:t},{binding:1,resource:e[n]},{binding:2,resource:e[r]},{binding:3,resource:l.velocity.view},{binding:4,resource:l.density.view}],`${i}-update-${n}-to-${r}`),L=Ve(c,`ParticlesScene`),R=c.gpu.createBindGroupLayout({label:`ParticlesStorageLayout`,entries:[{binding:0,visibility:GPUShaderStage.VERTEX,buffer:{type:`read-only-storage`}}]}),z=c.gpu.createBindGroupLayout({label:`ParticlesShadowLayout`,entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:`depth`}},{binding:2,visibility:GPUShaderStage.FRAGMENT,sampler:{type:`comparison`}}]}),B=c.gpu.createPipelineLayout({label:`ParticlesShadowPipelineLayout`,bindGroupLayouts:[L.bindGroupLayout,R]}),ae=c.gpu.createPipelineLayout({label:`ParticlesPipelineLayout`,bindGroupLayouts:[L.bindGroupLayout,R,z]}),oe=new Ne(c,jt,{label:`ParticlesShadowDraw`,layout:B,vertexBuffers:_.getVertexLayouts(),...Ie({cullMode:`none`,depthFormat:`depth32float`})}),se=new Ne(c,kt,{label:`CubesShadowDraw`,layout:B,vertexBuffers:S.getVertexLayouts(),...Ie({cullMode:`back`,depthFormat:`depth32float`})}),ce=new Me(c,Nt,{label:`ParticlesDraw`,layout:ae,vertexBuffers:_.getVertexLayouts(),...Fe({cullMode:`none`,colorFormat:c.format})}),ue=new Me(c,Pt,{label:`CubesDraw`,layout:ae,vertexBuffers:S.getVertexLayouts(),...Fe({cullMode:`back`,colorFormat:c.format})}),de=q.create(c,L.bindGroupLayout,C,0,`scene-bind-group`),fe=q.create(c,L.bindGroupLayout,M,0,`light-scene-bind-group`),pe=m.map((e,t)=>q.create(c,R,[{binding:0,resource:e}],`particle-draw-${t}`)),me=h.map((e,t)=>q.create(c,R,[{binding:0,resource:e}],`cube-draw-${t}`)),ge=Le.create(c,{label:`ParticlesShadowMap`,size:zt,format:`depth32float`}),_e=K.create(c,K.uniformSize(80),G.uniform,`shadow-uniforms`),V=new Float32Array(20);V.set(j.getViewProjectionMatrix(),0),V[16]=Ht,V[17]=zt,V[18]=Ut,_e.write(c,V);let ve=q.create(c,z,[{binding:0,resource:_e},{binding:1,resource:ge.view},{binding:2,resource:ge.sampler}],`shadow-bind-group`),H=null,U=0,W=0,Ce=0,we=performance.now(),Te=()=>{e.width===U&&e.height===W||(U=e.width,W=e.height,U>0&&W>0&&(T.setAspect(U/W),D.resolution=[U,W]))},Ee=()=>{let t=e.width,n=e.height;return H&&H.width===t&&H.height===n?H.createView():(H?.destroy(),H=c.gpu.createTexture({label:`depth-texture`,size:[t,n],format:`depth24plus`,usage:GPUTextureUsage.RENDER_ATTACHMENT}),H.createView())},De=(e,t,n)=>{if(!O.showFluidSlice||!O.showSliceVelocity&&!O.showSliceDensity)return;let r=[{visible:O.showSliceDensity,plane:u,showVelocity:!1,showDensity:!0},{visible:O.showSliceVelocity,plane:d,showVelocity:!0,showDensity:!1}].filter(e=>e.visible),i=Math.max(140,Math.min(280,Math.floor(Math.min(t,n)*.24))),a=Math.max(14,n-i-14);for(let t=0;t<r.length;t++){let n=14+t*(i+10);e.setViewport(n,a,i,i,0,1),e.setScissorRect(n,a,i,i),r[t].plane.draw(e,l.velocity,l.density,P.getViewProjectionMatrix(),{showVelocity:r[t].showVelocity,showDensity:r[t].showDensity,densityGain:6,velocityThreshold:1.2})}},Oe=t=>{null?.begin(),c.resize(...an(e)),Te();let n=Math.min(1/30,Math.max(1/240,(t-we)/1e3));we=t;let r=t*.001;ne.set(`time`,r).set(`dt`,n).writeToBuffer(F,c),re.set(`time`,r).set(`dt`,n).writeToBuffer(te,c),T.writeUniformData(w),C.write(c,w);let i=1-Ce,a=c.getCurrentTexture().createView(),o=Ee(),s=c.gpu.createCommandEncoder({label:`fluid-particles-frame`});ee(r),l.update(s,n);let u=s.beginComputePass({label:`update-particles`});ie.dispatch(u,I(m,F,Ce,i,`particles`),Math.ceil(Q/It)),ie.dispatch(u,I(h,te,Ce,i,`cubes`),Math.ceil(Ft/It)),u.end();let d=ge.beginRenderPass(s);oe.draw(d,_,[fe,pe[i]],Q),se.draw(d,S,[fe,me[i]],Ft),d.end();let f=Ae(s,a,{clearColor:{r:.015,g:.014,b:.012,a:1},depthStencilAttachment:{view:o,depthLoadOp:`clear`,depthClearValue:1,depthStoreOp:`store`}});ce.draw(f,_,[de,pe[i],ve],Q),ue.draw(f,S,[de,me[i],ve],Ft),f.end();let p=Ae(s,a,{loadOp:`load`,depthStencilAttachment:{view:o,depthLoadOp:`clear`,depthClearValue:1,depthStoreOp:`store`}});De(p,e.width,e.height),p.end(),c.gpu.queue.submit([s.finish()]),Ce=i,null?.end(),requestAnimationFrame(Oe)};window.addEventListener(`beforeunload`,()=>{D.disconnect(),E.destroy(),null?.destroy(),u.destroy(),d.destroy(),l.destroy(),H?.destroy(),C.destroy(),M.destroy(),F.destroy(),te.destroy(),_e.destroy(),ge.destroy(),g.destroy(),y.destroy(),b.destroy(),x.destroy(),m.forEach(e=>e.destroy()),h.forEach(e=>e.destroy())}),requestAnimationFrame(Oe)}ln().catch(e=>{console.error(e)});