...
`,Xt=`${He}\n${qt}`,Z=5e5,Zt=256,Q=9,Qt=64,$t=Q*2,$={strength:150,radius:6,swirlStrength:.9,swirlRadius:Q*.35,updraft:.55,forceCount:8,noiseStrength:.35},en=1024,tn=1.35,nn=.75,rn=.65,an=.002,on=[1,18,8],sn=[0,0,-1],cn=Q*.75,ln=Math.PI/180;async function un(){await Ze();let e=document.createElement(`canvas`);e.style.cssText=`display:block;width:100vw;height:100vh;touch-action:none;`,document.body.appendChild(e);let t=new Ht.default;t.showPanel(0),t.dom.style.cssText=`position:fixed;top:0;left:0;z-index:10;`,document.body.appendChild(t.dom);let n=document.createElement(`div`);n.textContent=`${Z.toLocaleString()} fluid particles`,n.style.cssText=`position:fixed;right:14px;bottom:12px;z-index:10;color:#d8d8d8;font:12px/1.45 ui-sans-serif,system-ui,sans-serif;letter-spacing:0;text-shadow:0 1px 2px rgba(0,0,0,0.7);pointer-events:none;user-select:none;`,document.body.appendChild(n);let r=await De.create(e),i=new ht(r,{TEXTURE_SIZE:Qt,DENSITY_DISSIPATION:.95,VELOCITY_DISSIPATION:.98,PRESSURE_DISSIPATION:.95,PRESSURE_ITERATIONS:24,CURL:6,ADVECTION_SCALE:16},Q),a=new vt(r,{texSize:Qt,volumeExtent:$t}),o=new vt(r,{texSize:Qt,volumeExtent:$t}),s=Ut({count:Z,radius:Q}),c=[G.fromData(r,s,W.storage,`particles-a`),G.fromData(r,s,W.storage,`particles-b`)],{positions:l}=Ye(1,1,1,`xy`),u=G.fromData(r,l,W.vertex,`particle-quad-positions`),d=new Ue(l.length/3).addVertexBuffer({buffer:u,arrayStride:12,attributes:[{shaderLocation:0,format:`float32x3`,offset:0}],slot:0,stepMode:`vertex`}),f=G.create(r,G.uniformSize(we.uniformByteSize()),W.uniform,`camera-uniforms`),p=new Float32Array(we.uniformFloatCount),m=new we(45*ln,1,.1,300),h=new he(m,{listenerTarget:e,center:[0,0,0],radius:Q*4,sensitivity:1,zoomSpeed:.8,panSpeed:.02});h.rx.setTo(-.22),h.ry.setTo(.72);let g=new ye(Je.sphere({radius:cn,segments:24}),m,[e.width,e.height],{listenerTarget:e}),_={advectionScale:16,curl:i.settings.CURL,densityDissipation:i.settings.DENSITY_DISSIPATION,velocityDissipation:i.settings.VELOCITY_DISSIPATION,pressureIterations:i.settings.PRESSURE_ITERATIONS,showFluidSlice:!0,showSliceVelocity:!0,showSliceDensity:!0},v=new Vt({title:`Fluid Particles`});v.add(_,`advectionScale`,1,64,1).name(`Advection scale`).onChange(e=>{i.settings.ADVECTION_SCALE=e}),v.add(_,`curl`,0,60,1).name(`Curl (vorticity)`).onChange(e=>{i.settings.CURL=e}),v.add(_,`densityDissipation`,.9,1,.001).name(`Density decay`).onChange(e=>{i.settings.DENSITY_DISSIPATION=e}),v.add(_,`velocityDissipation`,.9,1,.001).name(`Velocity decay`).onChange(e=>{i.settings.VELOCITY_DISSIPATION=e}),v.add(_,`pressureIterations`,1,40,1).name(`Pressure iters`).onChange(e=>{i.settings.PRESSURE_ITERATIONS=e}),v.add(_,`showFluidSlice`).name(`Show fluid slice`),v.add(_,`showSliceVelocity`).name(`Slice velocity`),v.add(_,`showSliceDensity`).name(`Slice density`);let y=!0,b=bt();g.addEventListener(`onHit`,(e=>{let t=e.detail.hit;if(y){Ct(b,t),y=!1;return}let n=bt();Et(n,t,b);let r=St(n);if(r>.001&&r<Q){let e=xt(n);Tt(e,e);let a=$.strength*(r*500);i.addForce([t[0],t[1],t[2]],[e[0],e[1],e[2]],$.radius,a,1,$.noiseStrength)}Ct(b,t)}));let x=new Te(-1,1,-1,1,.1,100);Ee({camera:x,center:[0,0,0],radius:Kt({maxRadius:Q,overshootMultiplier:tn,billboardPadding:nn}),eye:on,up:sn,padding:1});let S=G.create(r,G.uniformSize(Te.uniformByteSize()),W.uniform,`light-camera-uniforms`),C=new Float32Array(Te.uniformFloatCount);x.writeUniformData(C),S.write(r,C);let w=new Te(-1,1,-1,1,.1,100);Ee({camera:w,center:[0,0,0],radius:Q,eye:[0,0,Q*3],up:[0,1,0]});let T=G.create(r,32,W.uniform,`sim-params`),E=new ArrayBuffer(32),D=new Float32Array(E),O=new Uint32Array(E);D[2]=Q*1.25,O[3]=Z,D[4]=2.8,D[5]=.02,D[6]=.998,D[7]=8.4;let k=new J(r,Yt,{label:`ParticlesUpdate`,entryPoint:`cs_main`}),A=(e,t)=>K.create(r,k.getBindGroupLayout(0),[{binding:0,resource:T},{binding:1,resource:c[e]},{binding:2,resource:c[t]},{binding:3,resource:i.velocity.view},{binding:4,resource:i.density.view}],`particles-update-${e}-to-${t}`),j=Ge(r,`ParticlesScene`),M=r.gpu.createBindGroupLayout({label:`ParticlesStorageLayout`,entries:[{binding:0,visibility:GPUShaderStage.VERTEX,buffer:{type:`read-only-storage`}}]}),N=r.gpu.createBindGroupLayout({label:`ParticlesShadowLayout`,entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:`depth`}},{binding:2,visibility:GPUShaderStage.FRAGMENT,sampler:{type:`comparison`}}]}),P=r.gpu.createPipelineLayout({label:`ParticlesShadowPipelineLayout`,bindGroupLayouts:[j.bindGroupLayout,M]}),ee=r.gpu.createPipelineLayout({label:`ParticlesPipelineLayout`,bindGroupLayouts:[j.bindGroupLayout,M,N]}),te=new Be(r,Jt,{label:`ParticlesShadowDraw`,layout:P,vertexBuffers:d.getVertexLayouts(),primitive:{topology:`triangle-list`,cullMode:`none`},depthFormat:`depth32float`,depthWriteEnabled:!0,depthCompare:`less`}),ne=new ze(r,Xt,{label:`ParticlesDraw`,layout:ee,vertexBuffers:d.getVertexLayouts(),primitive:{topology:`triangle-list`,cullMode:`none`},depthStencil:{format:`depth24plus`,depthWriteEnabled:!0,depthCompare:`less`},targets:[{format:r.format}]}),re=K.create(r,j.bindGroupLayout,f,0,`scene-bind-group`),ie=K.create(r,j.bindGroupLayout,S,0,`light-scene-bind-group`),ae=c.map((e,t)=>K.create(r,M,[{binding:0,resource:e}],`particle-draw-${t}`)),F=Ve.create(r,{label:`ParticlesShadowMap`,size:en,format:`depth32float`}),I=G.create(r,G.uniformSize(80),W.uniform,`shadow-uniforms`),L=new Float32Array(20);L.set(x.getViewProjectionMatrix(),0),L[16]=rn,L[17]=en,L[18]=an,I.write(r,L);let R=K.create(r,N,[{binding:0,resource:I},{binding:1,resource:F.view},{binding:2,resource:F.sampler}],`shadow-bind-group`),z=null,B=0,oe=0,se=0,ce=performance.now(),le=()=>{e.width===B&&e.height===oe||(B=e.width,oe=e.height,B>0&&oe>0&&(m.setAspect(B/oe),g.resolution=[B,oe]))},V=()=>{let t=e.width,n=e.height;return z&&z.width===t&&z.height===n?z.createView():(z?.destroy(),z=r.gpu.createTexture({label:`depth-texture`,size:[t,n],format:`depth24plus`,usage:GPUTextureUsage.RENDER_ATTACHMENT}),z.createView())},H=e=>{if(i.addForce([0,0,0],[0,1,0],$.radius,$.strength*$.updraft,1.5,.25),$.swirlStrength<=0)return;let t=Math.max(1,Math.floor($.forceCount)),n=e*.65;for(let e=0;e<t;e++){let r=n+e/t*Math.PI*2,a=Math.cos(r),o=Math.sin(r),s=[a*$.swirlRadius,0,o*$.swirlRadius],c=[-o*$.swirlStrength,$.updraft,a*$.swirlStrength],l=Math.hypot(c[0],c[1],c[2]);i.addForce(s,[c[0]/l,c[1]/l,c[2]/l],$.radius,$.strength/t,.35,$.noiseStrength)}},ue=(e,t,n)=>{if(!_.showFluidSlice||!_.showSliceVelocity&&!_.showSliceDensity)return;let r=[{visible:_.showSliceDensity,plane:a,showVelocity:!1,showDensity:!0},{visible:_.showSliceVelocity,plane:o,showVelocity:!0,showDensity:!1}].filter(e=>e.visible),s=Math.max(140,Math.min(280,Math.floor(Math.min(t,n)*.24))),c=Math.max(14,n-s-14);for(let t=0;t<r.length;t++){let n=14+t*(s+10);e.setViewport(n,c,s,s,0,1),e.setScissorRect(n,c,s,s),r[t].plane.draw(e,i.velocity,i.density,w.getViewProjectionMatrix(),{showVelocity:r[t].showVelocity,showDensity:r[t].showDensity,densityGain:6,velocityThreshold:1.2})}},U=n=>{t.begin(),r.resize(),le();let a=Math.min(1/30,Math.max(1/240,(n-ce)/1e3));ce=n,D[0]=n*.001,D[1]=a,T.write(r,E),m.writeUniformData(p),f.write(r,p);let o=1-se,s=r.getCurrentTexture().createView(),c=V(),l=r.gpu.createCommandEncoder({label:`fluid-particles-frame`});H(n*.001),i.update(l,a);let u=l.beginComputePass({label:`update-particles`});k.dispatch(u,A(se,o),Math.ceil(Z/Zt)),u.end();let h=F.beginRenderPass(l);te.draw(h,d,[ie,ae[o]],Z),h.end();let g=Le(l,s,{clearColor:{r:.015,g:.014,b:.012,a:1},depthStencilAttachment:{view:c,depthLoadOp:`clear`,depthClearValue:1,depthStoreOp:`store`}});ne.draw(g,d,[re,ae[o],R],Z),g.end();let _=Le(l,s,{loadOp:`load`,depthStencilAttachment:{view:c,depthLoadOp:`clear`,depthClearValue:1,depthStoreOp:`store`}});ue(_,e.width,e.height),_.end(),r.gpu.queue.submit([l.finish()]),se=o,t.end(),requestAnimationFrame(U)};window.addEventListener(`beforeunload`,()=>{g.disconnect(),h.destroy(),v.destroy(),a.destroy(),o.destroy(),i.destroy(),z?.destroy(),f.destroy(),S.destroy(),T.destroy(),I.destroy(),F.destroy(),u.destroy(),c.forEach(e=>e.destroy())}),requestAnimationFrame(U)}un().catch(e=>{console.error(e)});