...
function randomUnitVec2(): [number, number] {
...
export default class FluidSimulation {
  ...
  private readonly _velocity: Texture2DPingPong;
  private readonly _density: Texture2DPingPong;
  private readonly _pressure: Texture2DPingPong;
  private readonly _divergenceTex: Texture2D;
  ...
  constructor(
    ...
    this._gridSize = size;
    this._dispatch = [size / WORKGROUP_SIZE, size / WORKGROUP_SIZE, 1];

    this._velocity = Texture2DPingPong.create(device, size, { label: "Velocity" });
    this._density = Texture2DPingPong.create(device, size, { label: "Density" });
    this._pressure = Texture2DPingPong.create(device, size, { label: "Pressure" });
    this._divergenceTex = Texture2D.create(device, size, { label: "Divergence" });
  ...
  applyRandomForces(count: number, options: RandomForceOptions = {}): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.addForce(worldPos, randomUnitVec2(), radius * this.maxRadius, strength);
    }
  }

  updateFlowWithMap(
    _mTextureVel: Texture2D,
    _mTextureDensity: Texture2D,
    _uStrength = 1,
  ): void {
  ...
  get velocity(): Texture2D {
    return this._velocity.read;
  }

  get density(): Texture2D {
    return this._density.read;
  }

  get divergence(): Texture2D {
    return this._divergenceTex;
  }

  get pressure(): Texture2D {
    return this._pressure.read;
  }

  get allTextures(): Texture2D[] {
  ...
  private _advectPass(
    ...
    pingPong: Texture2DPingPong,
    dissipation: number,
  ): void {
    this._writePassUniforms(Math.pow(dissipation, this._dt * 60.0));
  ...
  private _flushForces(pass: GPUComputePassEncoder): void {
    for (let i = 0; i < this._pendingForces.length; i++) {
      ...
      forceUniforms
        ...
        .set("center", [force.center[0], force.center[1], 0, 0])
        .set("dir", [force.dir[0], force.dir[1], 0, force.noiseStrength])
  ...
  private _clearPass(pass: GPUComputePassEncoder): void {
    clearUniforms
      .set("gridSize", this._gridSize)
      .set("dissipation", Math.pow(this.settings.PRESSURE_DISSIPATION, this._dt * 60.0))
      .set("_pad0", 0)
      .set("_pad1", 0)
