...
class FaceDetection extends EventDispatcher {
      ...
      navigator.mediaDevices.enumerateDevices()
        .then(function (devices) {
          ...
          if (webcams.length === 1) {
            ...
            const logitech = webcams.filter(webcam => webcam.label.indexOf('C920') > -1)[0]
          ...
          window.navigator.mediaDevices.getUserMedia({ video: { width: 640, height: 480, frameRate: 30, deviceId } })
            .then(onStreamFetched).catch(function () { console.error('No camera available.') })
        })
        .catch(function (err) {
  ...
  async checkFace () {
    ...
    if (faces.length > 0) {
      const vertices = faces[0].scaledMesh.map(v => [-v[0] + videoWidth / 2, -v[1] + videoHeight / 2, -v[2]])
