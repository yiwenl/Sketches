...
uniform samplerCube uRadianceMapCurr;
uniform samplerCube uIrradianceMapCurr;
uniform samplerCube uRadianceMapNext;
uniform samplerCube uIrradianceMapNext;
...
vec3 getPbr(vec3 N, vec3 V, vec3 baseColor, float roughness, float metallic, float specular) {
	...
	vec3 radianceCurr	= pow( textureCubeLodEXT( uRadianceMapCurr, lookup, mip ).rgb, vec3( 2.2 ) );
	vec3 radianceNext	= pow( textureCubeLodEXT( uRadianceMapNext, lookup, mip ).rgb, vec3( 2.2 ) );
	vec3 radiance		= mix(radianceCurr, radianceNext, uOffset);
	vec3 irradianceCurr	= pow( textureCube( uIrradianceMapCurr, N ).rgb, vec3( 1 ) );
	vec3 irradianceNext	= pow( textureCube( uIrradianceMapNext, N ).rgb, vec3( 1 ) );
	vec3 irradiance		= mix(irradianceCurr, irradianceNext, uOffset);
