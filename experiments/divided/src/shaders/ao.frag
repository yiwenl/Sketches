...
{
    float aoCap = 1.0;
    float aoMultiplier = 50.0 * uAoRange;
...
float ssao() {
    ...
    // float depthThreshold = 0.2;  // Threshold for depth difference
    ...
    for (int i = 0; i < rings; i += 1) {
        ...
        for (int j = 0; j < samples * rings; j += 1) {
            ...
            // Check the depth difference to avoid occlusion on the background
            float depthDiff = abs(depth - d);
