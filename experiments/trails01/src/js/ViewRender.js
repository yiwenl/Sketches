...
class ViewRender extends alfrid.View {
  ...
  render (textureCurr, textureNext, p, textureExtra, mShadowMatrix, mTextureDepth, textureParticle, textureData) {
    this.time += 0.1
    this.shader.bind()
    ...
    this.shader.uniform('textureData', 'uniform1i', 5)
    textureData.bind(5)

    this.shader.uniform('textureMap', 'uniform1i', 6)
    Assets.get(Config.source).bind(6)
