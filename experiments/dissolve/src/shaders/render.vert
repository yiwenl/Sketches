...
void main(void) {
	...
	vec3 save    = texture2D(textureSave, uv).rgb;
	vec4 wsPosition  = uModelMatrix * vec4(pos, 1.0);
	gl_Position  = uProjectionMatrix * uViewMatrix * wsPosition;
	

	float g 	 = mix(extra.r, 1.0, .75);
	vColor       = vec4(vec3(g), save.y);

	float distOffset = uViewport.y * uProjectionMatrix[1][1] * radius / gl_Position.w;
    // gl_PointSize = distOffset * (1.0 + extra.x * 1.0);

    float a = smoothstep(0.0, 0.5, vColor.a);

    gl_PointSize = distOffset * extra.x * a;
