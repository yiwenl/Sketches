...
void main(void) {
	...
	vec3 save            = texture2D(textureSave, vTextureCoord).rgb;
	float life 			 = save.y;
	...
	float isIn 			 = 1.0;
	float offset   		 = 0.1;
	if(pos.z > color0.z + offset) {
		isIn = 0.0;
	} else if(pos.z < color1.z - offset) {
		isIn = 0.0;
	}
	if(color0.a * color1.a <= 0.0) {
		isIn = 0.0;
	}


	float posOffset      = mix(extra.r, 1.0, .75) * 0.25;
	vec3 acc             = curlNoise(pos * posOffset + time * .5);
	acc.y 				 = (acc.y + 0.75) * 0.5;
	acc.xz 				 -= vec2(0.25);
	float speedOffset    = mix(extra.g, 1.0, .5);

	float lifeOffset     = smoothstep(0.5, 0.25, life);
	if(life <= 0.0) {
		lifeOffset = 0.0;
	}

	
	vel                  += acc * .003 * speedOffset * lifeOffset;
	pos                  += vel;
	...
	if(life <= 0.0) {
		life = 0.0;
		float dist = distance(orgPos, uHit);
		if(dist > uHitRadius) {
			pos = orgPos;
			save.b = 1.0;
		} else {
	...
	if(save.x < 0.5) {
		if(isIn > 0.5) {
			...
			const vec3 bound = vec3(2.0, 6, 2.0);
			pos.x = snoise(pos + time);
			pos.y = snoise(pos.yzx + time);
			pos.z = snoise(pos.zxy + time);
			pos *= bound;
			pos.y += bound.y;
		}	
	} 
