...
class CameraPerspective extends Camera {
  ...
  /**
   * Get the field of view (FOV) in degrees of the perspective camera.
   * @returns {number} The field of view in degrees.
   */
  get fov() {
    const yScale = this._mtxProj[5]; // Element at second row, second column
    return 2 * Math.atan(1 / yScale);
  }
