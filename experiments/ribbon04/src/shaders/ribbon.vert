...
float getLife(float mIndex, vec2 mUV) {
    float index = mIndex + uIndex;
    index = mod(index, uTotal);
...
void main(void) {
    ...
    // debugging
    float life = getLife(index, uv);
    float lifeNext = getLife(index + 1.0, uv);
    float lifePrev = getLife(index - 1.0, uv);
    vSkip = step(lifePrev, life) + step(life, lifeNext);
