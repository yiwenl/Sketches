...
void main() {
	...
	// float r 			= 0.1;
	float c 			= 0.3;
	float noise 		= snoise(vWsPosition * 1.1 + uTime);
	noise 				= smoothstep(c-r, c+r, noise);
	vec3 colorNoise     = texture2D( uNoiseMap, vTextureCoord * 5.0).rgb;

	vec3 N 				= normalize( vWsNormal + colorNoise * noise * 0.5);
	vec3 V 				= normalize( vEyePosition );

	vec3 colorGold 		= vec3(1.000, 0.766, 0.276);
	
	
	vec3 baseColor 	    = mix(uBaseColor, colorGold, noise);
	float roughness     = clamp(uRoughness + (1.0 - noise), 0.0, 1.0);
	float specular 		= uSpecular - (1.0 - noise) * 0.95;

	vec3 color 			= getPbr(N, V, baseColor, roughness, noise * uMetallic, specular);
	// vec3 color 			= getPbr(N, V, baseColor, roughness, uMetallic, specular);
