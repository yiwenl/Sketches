...
void main(void) {
	...
	vec3 spherePosition		= normalize(aVertexPosition);
	position 				= mix(aVertexPosition, spherePosition, uOffset);

	vec4 worldSpacePosition	= uModelMatrix * vec4(position, 1.0);
    vec4 viewSpacePosition	= uViewMatrix * worldSpacePosition;

    vNormal					= uNormalMatrix * mix(aNormal, spherePosition, uOffset);
