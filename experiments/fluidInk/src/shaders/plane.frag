...
void main() {
	...
	vec3 N1 				= normalize(colorNormal);
	vec3 V 				= normalize( vEyePosition );

	vec2 uv = envMapEquirect(N);
	vec4 color = texture2D(textureNoise, uv);

	vec3 noise = texture2D(textureNoise, vTextureCoord * 0.5).rgb;
	N = normalize(N + (noise - .5) * 0.4);


    vec3 lookup = reflect(V, N);
    vec3 lookup1 = reflect(V, N1);
    color = textureCube(uRadianceMap, lookup); 
    color.rgb *= 0.25;
    float d = diffuse(N, LIGHT);
    color.rgb *= d;
    // vec3 color1 = textureCube(uIrradianceMap, lookup).rgb; 
    // color.rgb *= 1.0 - color1;
