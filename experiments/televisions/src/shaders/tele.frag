...
void main() {
	vec3 colorNormal 	= texture2D( uNormalMap, vTextureCoord).rgb * 0.5;
	vec3 N 				= normalize( vWsNormal + colorNormal );
	vec3 V 				= normalize( vEyePosition );
	
	vec3 maps 			= texture2D(uMaps, vTextureCoord).rgb;	
	vec3 baseColor 		= texture2D( uColorMap, vTextureCoord).rgb * 0.25;
	float roughness 	= maps.b;
	float metallic 		= maps.g;
	...
	//	glow			
	color.rgb 			+= texture2D( uSpecularMap, vTextureCoord).rgb;
