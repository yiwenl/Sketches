...
vec3 getNormal() {
...
#ifdef HAS_NORMALMAP
	vec3 n = texture2D(uNormalMap, vTextureCoord * uUVScale).rgb;
	...
	n.yz = rotate(n.yz, uEnvRot.x);
	n.xz = rotate(n.xz, uEnvRot.y);
...
void main() {
...
#ifdef HAS_BASECOLORMAP	
	vec4 baseColor = SRGBtoLINEAR(texture2D(uColorMap, vTextureCoord * uUVScale));
#else
...
#ifdef HAS_OCCLUSIONMAP	
	float ao            = texture2D(uAoMap, vTextureCoord * uUVScale).r;
	color               = mix(color, color * ao, uOcclusionStrength);
#endif	

#ifdef HAS_EMISSIVEMAP
    vec3 emissive = SRGBtoLINEAR(texture2D(uEmissiveMap, vTextureCoord * uUVScale)).rgb * uEmissiveFactor;
    color += emissive;
#endif
	...
	float s             = PCFShadow(uShadowMap, uMapSize, shadowCoord);
	s = smoothstep(0.0, 0.7, s);
