...
class FaceTracking extends Emitter {
		...
		const startCamera = () => new Promise( (resolve, reject) => {
			...
			.then(function(devices) {
				...
				window.navigator.mediaDevices.getUserMedia({ video: { width: 640, height: 480, frameRate: 30} })
		      	.then(onStreamFetched).catch(function (o) { alert("No camera available.", o); });
			})
			.catch(function(err) {
	...
	trackFaces() {
		...
		const tx = x * TextureManager.canvasScale;
		const ty = y * TextureManager.canvasScale;
