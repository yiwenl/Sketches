...
class SceneApp extends Scene {
  constructor() {
    ...
    this._handLost = true;
    this._glow = new EaseNumber(1, 0.2);
    this._timeoutGlow0 = -1;
    this._timeoutGlow1 = -1;
    setTimeout(() => this.pulse(), randomInt(2000, 1000));
  }

  pulse() {
    window.clearTimeout(this._timeoutGlow0);
    window.clearTimeout(this._timeoutGlow1);

    if (!this._handLost) this._glow.setTo(random(4, 5));

    this._timeoutGlow0 = setTimeout(() => {
      this._glow.value = 1;
    }, 500);

    this._timeoutGlow1 = setTimeout(() => this.pulse(), randomInt(2000, 3000));
  }

  _onHandsDetected = (hands) => {
    const wasLost = this._handLost;
    ...
    // const handLeft = hands.find(
    //   ({ handedness }) => handedness.toLowerCase() === "left"
    // );
    // if (handLeft) {
    //   const { keypoints, keypoints3D } = handLeft;
    //   const pointBase = keypoints.find((p) => p.name === base);
    //   const thumbTip = findJoint("thumb_tip", keypoints3D, pointBase);
    //   const indexTip = findJoint("index_finger_tip", keypoints3D, pointBase);
    // }
  };
  ...
  _initViews() {
    ...
    this._passBloom = new PassBloom(5, 0.5);
  ...
  update() {
    ...
    this._drawSim
      ...
      .uniform("uGlow", this._glow.value)
      .draw();
    ...
    for (let s in this.hands) {
      ...
      fingers.forEach((finger, i) => {
        ...
        if (len > thresholdMovement) {
          ...
          this._fluid.updateFlow(pos, dir, str, 2);
        }
  ...
  render() {
    ...
    this._drawCompose
      ...
      .bindTexture("uBloomMap", this._passBloom.texture, 1)
      .draw();
    // this._drawBg.draw();
    GL.enable(GL.DEPTH_TEST);
    // GL.setMatrices(this.camera);
