...
const vec3 LIGHT = vec3(0.5, 1.0, -1.0);
...
/*
	...
	const float posOffset = 0.25;
	float noise = snoise(vPosition*posOffset+uGlobalTime) * .5 + .5;
	noise = mix(noise, 1.0, .5);
	color.rgb = mix(color.rgb, fogColor, noise);
*/
	gl_FragColor = color;
}