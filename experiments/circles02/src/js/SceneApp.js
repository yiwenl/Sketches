...
import fsPortrait from 'shaders/portrait.frag'
...
class SceneApp extends Scene {
  ...
  _initTextures () {
    ...
    this.texture = Assets.get('portrait01')
    const ratio = this.texture.width / this.texture.height

    const size = 3
    this._drawPortrait = new alfrid.Draw()
      .setMesh(alfrid.Geom.plane(size, size / ratio, 1, 'xy'))
      .useProgram(null, fsPortrait)
      .uniformTexture('texture', this.texture, 0)
  }
  ...
  update () {
    ...
    const draw = this._drawPortrait

    draw.bindFrameBuffer(this._fbo)
      .uniform('uContrast', 'float', Config.contrast)
