...
class TouchSimulator extends EventDispatcher {
	constructor(mRadiusMultiplier=1, mResetting=true) {
		...
		this._position = vec3.fromValues(
			random(r, 1.0-r) * Config.TEXTURE_WIDTH, 
			random(r, 1.0-r) * Config.TEXTURE_HEIGHT, 
			0
		);

		this._position[0] = Config.TEXTURE_WIDTH * 0.8;
		this._position[1] = Config.TEXTURE_HEIGHT/2;
		this._reset();
	...
	_checkBound() {
		//	bouncing
		//*/
		if(this._position[0] > Config.TEXTURE_WIDTH) {
			this._position[0] -= Config.TEXTURE_WIDTH;
