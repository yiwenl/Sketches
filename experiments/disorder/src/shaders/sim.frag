...
void main(void) {
    ...
    float t = 0.95;
    float _f = fract(extra.y + extra.z);
    _f = smoothstep(t, t + 0.01, _f);


    // apply force
    float speed = mix(1.0, 4.0, extra.x) * (1.0 + _f) * 4.0;
    vel += acc * 0.001 * uSpeed;

    float maxSpeed = 0.002 * uMaxSpeed;
    float _speed = length(vel);
    if(_speed > maxSpeed) {
    ...
    vec2 bound = uBound;
    t = fract(extra.x + extra.y + extra.z);
    // t = step(t, 0.02);
    t = mix(1.0, 1.1, t);
    bound *= t;
