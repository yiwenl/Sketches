...
export default (scene) => {
  ...
  gui.add(Config, "numParticles", [256, 384, 512]).onFinishChange(reload);
  gui.add(Config, "debugAvoidCenters").onFinishChange(refresh);
  gui.add(Config, "speed", 0.1, 5).onFinishChange(refresh);
  gui.add(Config, "decay", 0.7, 0.99).onFinishChange(refresh);

  gui.add(Config, "noiseScale", 0.1, 5).onFinishChange(refresh);
  gui.add(Config, "noiseSpeed", 0.1, 5).onFinishChange(refresh);
  gui.add(Config, "maxSpeed", 0.1, 5).onFinishChange(refresh);
  gui.add(Config, "rotationForce", 0.1, 2).onFinishChange(refresh);
  ...
  fSystem.add(Config, "showFps").onFinishChange(reload);
