...
class SceneApp extends Scene {
  constructor() {
    ...
    this.orbitalControl.radius.setTo(17);
  ...
  _initViews() {
    ...
    this._drawTest = new Draw()
      .setMesh(Geom.bigTriangle())
      .useProgram(ShaderLibs.bigTriangleVert, fsTest);
  }

  swirl() {
    const { rotationForce } = Config;
    const time = Scheduler.getElapsedTime() * 0.1;
    const num = 5;
    const { PI } = Math;
    for (let i = 0; i < num; i++) {
      const r = random(0.3, 0.5);
      ...
      this._fluid.updateFlow(pos, dir, 1 * rotationForce, 2 * rotationForce, 0);
    }
  }
