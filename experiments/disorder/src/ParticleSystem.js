...
export default class ParticleSystem {
  constructor(numParticles) {
    this.numParticles = parseInt(numParticles);
    ...
    const getCenter = () => {
      const rx = 3;
      const ry = 4;
      // return [random(-rx, rx), random(-ry, ry), random(1, 3)];
  ...
  update(mFluidMap, mDensityMap) {
    const { speed, decay, maxSpeed } = Config;
    this._drawSim
      ...
      .uniform("uDecay", decay)
      .uniform("uMaxSpeed", maxSpeed)
      .draw();
