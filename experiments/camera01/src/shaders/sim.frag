...
void main(void) {
	...
	float posOffset      = mix(extra.r, 1.0, .75) * .15;
	vec3 acc             = curlNoise(pos * posOffset + time * .5);
	float speedOffset    = mix(extra.g, 1.0, .5);


	vec2 accRot = normalize(pos.xz);
	accRot = rotate(accRot, PI * 0.6);

	acc.xz += accRot * 1.0;
	
	vec3 posAdj = vec3(pos);
	posAdj.y *= 1.75;
	float dist           = length(posAdj);
	if(dist > maxRadius) {
		float f          = pow(2.0, (dist - maxRadius) * 2.0) * 0.2;
		acc              -= normalize(posAdj) * f;
	}
