...
class FrameBuffer {
	...
	constructor(mWidth, mHeight, mParameters = {}, multipleTargets = false, numTargets = 4) {
		gl = GL.gl;
		...
		if(GL.webgl2) {
			...
			if(this._multipleTargets) {
				...
				for(let i=0; i<this._numTargets; i++) {
					attachments.push(extDrawBuffer[`COLOR_ATTACHMENT${i}_WEBGL`]);
				} 
				...
				// ]);	
				extDrawBuffer.drawBuffersWEBGL(attachments);	
			}
	...
	_initTextures() {
		const numTextures = this._multipleTargets ? this._numTargets : 1;
		for (let i = 0; i < 4; i++) {
